...
export function getDepartmentCode(codeCommune: string): string {
  if (codeCommune.startsWith('97')) return codeCommune.substring(0, 3);
  return codeCommune.substring(0, 2);
}

export function getDepartmentInfo(code: string): { name: string; region: string } | undefined {
  const dept = DEPARTMENTS[code];
  return dept ? { name: dept[0], region: dept[1] } : undefined;
}

export function getDepartmentEntries(): { code: string; name: string; region: string }[] {
  return Object.entries(DEPARTMENTS)
    .map(([code, [name, region]]) => ({ code, name, region }))
    .sort((a, b) => a.code.localeCompare(b.code));
}

export const REGIONS = [...new Set(Object.values(DEPARTMENTS).map(d => d[1]))]
